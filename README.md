...
当前版本中，书签按钮位于顶部工具栏，跟随当前选中的 Day 自动变化。

## 完成状态

每个 Day 底部有 `Mark as Done / Completed` 按钮。

- 点击 `Mark as Done` 后，该 Day 会被标记为已完成。
- 左侧 Day 导航会显示完成状态。
- 完成状态会保存到 `localStorage`，刷新后仍然保留。
- Day 底部还提供 `Previous Day` 和 `Next Day`，方便连续复习。
...
输入关键词后按 `Enter` 才会执行搜索，输入过程中不会立刻过滤页面。搜索会过滤显示相关 Day，并高亮匹配内容。若有多个命中，可以使用搜索框旁边的 `Previous` / `Next` 按钮在命中结果之间跳转；如果命中结果分布在不同 Day，页面会自动切换到对应 Day。`Clear` 可以清空搜索并恢复正常显示。如果没有匹配结果，页面会显示友好的空状态提示。
